 ...
 */
export const createDCAPlanTool: VibkitToolDefinition<
  any,
  any,
  DCAContext,
  any
> = {
  name: "createDCAPlan",
  description:
    'Create a new DCA plan database record only (without transaction preparation). Use this for basic plan creation when you do NOT need transaction data - e.g., "Create a DCA plan record", "Set up a new investment plan", "Add a plan to database"',
  ...
  execute: async (params: CreateDCAPlanRequest, context) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:${API_PORT}/api/dca/create`,
        {
          method: "POST",
          headers: {
      ...
      return createErrorTask(
        ...
        error instanceof Error
          ? error
          : new Error(`Failed to connect to DCA API: ${String(error)}`)
      );
    }
  },
};
...
export const getUserDCAPlans: VibkitToolDefinition<any, any> = {
  ...
  execute: async (params: GetUserDCAPlansRequest) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:${API_PORT}/api/dca/plans/${userAddress}`,
        {
          method: "GET",
          headers: {
...
export const updateDCAPlanStatus: VibkitToolDefinition<any, any> = {
  ...
  execute: async (params: UpdateDCAPlanStatusRequest) => {
    ...
    try {
      const API_PORT = parseInt(process.env.API_PORT || "3002", 10);
      const response = await fetch(
        `http://localhost:${API_PORT}/api/dca/plans/${planId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
...
export const getPlatformStats: VibkitToolDefinition<any, any> = {
  ...
  description:
    "Get overall platform statistics including total plans, users, and executions",
  parameters: GetPlatformStatsSchema,
  execute: async (params: GetPlatformStatsRequest) => {
    try {
      const API_PORT = parseInt(process.env.API_PORT || "3002", 10);
      const response = await fetch(
        `http://localhost:${API_PORT}/api/dca/stats`
      );
      const result = (await response.json()) as any;
